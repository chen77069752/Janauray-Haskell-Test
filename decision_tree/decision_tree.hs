...
lookUpAtt key xs row 
  = fromJust $ find (`elem` row) attValues
  where 
    attValues = lookUp key xs

removeAtt :: AttName -> Header -> Row -> Row
removeAtt key xs 
  = filter (`notElem` attValues)
  where 
    attValues = lookUp key xs 
...
addToMapping (x, v) xs 
  | isNothing n = (x, [v]) : xs
  | otherwise   = map (\(m, ms) -> if m == x then (m, v: ms) else (m, ms)) xs    
  where
    n = lookup x xs 

buildFrequencyTable :: Attribute -> DataSet -> [(AttValue, Int)]
--Pre: Each row of the data set contains an instance of the attribute
buildFrequencyTable a@(name, values) (header, rows)
  = map (\(a, l) -> (a, length l)) ms 
  where 
    valLs = map (lookUpAtt name header) rows 
    valPair = map (\x -> (x, [])) values 
    ls      = map (\x -> (x, 1)) valLs
    ms      = foldl (flip addToMapping) valPair ls 
...
nodes tree   
  | Null      <- tree = 0 
  | Leaf _    <- tree = 1 
  | Node _ xs <- tree = foldl (\acc (_, t) -> acc + nodes t) 1 xs

evalTree :: DecisionTree -> Header -> Row -> AttValue
evalTree tree header row  
  | Null <- tree 
    = "" 
  | Leaf value <- tree 
    = value 
...
partitionData (header, rows) a@(_, values) 
  = map (\(v, r) -> (v, (delete a header, r))) zs 
  where 
    ms = map (`reduce` rows) values 
    zs = zip values ms 
    reduce value rows 
      = map (filter (/= value)) $ filter (\row -> value `elem` row) rows 


buildTree :: DataSet -> Attribute -> AttSelector -> DecisionTree 
buildTree (header, rows) (name, _) _
  | all null rows          
    = Null  
  | allSame xs      
    = Leaf $ head xs  
    where 
      xs = map (lookUpAtt name header) rows 
buildTree d classifier selector
  = Node name subTrees 
  where 
    att@(name, _)  = selector d classifier 
    part = partitionData d att
    subTrees = map (\(value, d') -> (value, buildTree d' classifier selector)) part
