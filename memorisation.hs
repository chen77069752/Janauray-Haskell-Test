...
assignArray (A xs) (I i) (I v) 
  = A $ map (\(a, b) -> if a == i then (a, v) else (a, b)) xs

updateVar :: (Id, Value) -> State -> State
updateVar (id, v) state
  | isNothing maybeV 
    = nub $ (id, (Local, v)) : state 
  | otherwise 
    = nub $ map (\s@(i, (scope, _)) -> if i == id 
                    then (id, (scope, v)) 
                    else s) state 
    where 
      maybeV = lookup id state  
...
applyOp Less (I x) (I y)
  = if x < y then I 1 else I 0
applyOp Equal (I x) (I y)
  = if x == y then I 1 else I 0 
applyOp Index (A xs) (I x)    
  = I $ fromMaybe 0 (lookup x xs)      

bindArgs :: [Id] -> [Value] -> State
-- Pre: the lists have the same length
bindArgs ids vs 
  = map (\(i, v) -> (i, (Local, v))) (zip ids vs)

evalArgs :: [Exp] -> [FunDef] -> State -> [Value]
evalArgs es defs state 
  = map (\e -> eval e defs state) es
...
eval exp defs state 
  ...
  | Var x <- exp 
    = getValue x state 
  | Cond e1 e2 e3 <- exp 
    = if eval' e1 == I 1 then eval' e2 else eval' e3 
  | OpApp op e1 e2 <- exp 
    = applyOp op (eval' e1) (eval' e2)
    where 
      eval' e = eval e defs state  
eval (FunApp f es) defs state 
  = eval e defs state' 
  where 
    (as, e) = lookUp f defs
    vs      = evalArgs es defs state 
...
executeStatement sm fDefs pDefs state 
  | Assign id exp <- sm
    = updateVar (id, eval exp fDefs state) state
  | AssignA id e1 e2 <- sm
    = updateVar (id, assignArray (getValue id state) 
                                 (eval e1 fDefs state)
                                 (eval e2 fDefs state )) state 
  | If exp b1 b2 <- sm 
    = if eval exp fDefs state == I 1 then executeBlock b1 fDefs pDefs state 
                                     else executeBlock b2 fDefs pDefs state 
  | While exp b <- sm 
    = if eval exp fDefs state == I 1 then executeStatement sm fDefs pDefs (executeBlock b fDefs pDefs state) 
                                     else state  
  | Return exp <- sm 
    = updateVar ("$res", eval exp fDefs state) state   

executeStatement (Call r f es) fDefs pDefs state 
  | null r    = getGlobals state' ++ getLocals state
  | otherwise = updateVar (r, getValue "$res" state') (getGlobals state' ++ getLocals state)  
  where       
    (ids, b) = lookUp f pDefs
    vs       = evalArgs es fDefs state   
    bs       = bindArgs ids vs                          
    state'   = executeBlock b fDefs pDefs (bs ++ state)                             
...
executeBlock [] _ _ state 
  = state 
executeBlock (s : b) fDefs pDefs state 
  = executeBlock b fDefs pDefs state'  
  where 
    state' = executeStatement s fDefs pDefs state       
