...
update (key, n) state 
  | isNothing $ lookup key state 
    = (key, n) : state 
  | otherwise 
    = map (\(id, x) -> if id == key then (id, n) else (id, x)) state  
...
execBlock xs state 
  = foldr execStatement state (reverse xs)  
...
sub id n exp 
  | Var v <- exp, v == id
    = Const n 
  | Phi e1 e2 <- exp 
    = foldConst $ Phi (sub id n e1) (sub id n e2) 
  | Apply op e1 e2 <- exp 
    = foldConst $ Apply op (sub id n e1) (sub id n e2) 
  | otherwise 
    = exp       
...
propagateConstants2 :: Block -> Block
-- Pre: the block is in SSA form
propagateConstants2 b 
  = propagateConstants2' initB initF
  where 
    (initF, initB) = scan' id b  
    propagateConstants2' b f
      | b == b' 
        = b
      | otherwise
        = propagateConstants2' b' f'
...
scan' f (Assign x exp : xs)
  ...
  | otherwise 
    ...
    where 
      (f', b) = scan' f xs
scan' f (If exp b1 b2 : xs)
  = (f' . f'' . f''', If exp b b' : xs')
  where
    (f', b)     = scan' f b1   
    (f'', b')   = scan' f b2   
    (f''', xs') = scan' f xs  
scan' f (DoWhile b exp : xs) 
  = (f' . f'', DoWhile b' exp :  xs')
  where 
    (f', b')   = scan' f b  
    (f'', xs') = scan' f xs           
...
propagateConstants b 
  = propagateConstants' initB initWl
  where 
    (initWl, initB) = scan "$NotValid" 0 b  
    propagateConstants' b [] 
      = b 
    propagateConstants' b ((id, n) : wl)
...
scan _ _ [] 
  = ([], [])
scan id n (Assign x exp : xs)
  | x == "$return" 
    = (wl, Assign x exp : b) 
  | Const m <- sub id n exp
    = ((x, m) : wl, b)
  | otherwise 
    = (wl, Assign x (sub id n exp) : b)  
    where 
      (wl, b) = scan id n xs    
scan id n (If exp b1 b2 : xs)
  = (wl ++ wl' ++ wl'', If exp b b' : xs')
  where
    (wl, b)     = scan id n b1   
    (wl', b')   = scan id n b2   
    (wl'', xs') = scan id n xs  
scan id n (DoWhile b exp : xs) 
  = (wl ++ wl', DoWhile b' exp :  xs')
  where 
    (wl, b')   = scan id n b  
    (wl', xs') = scan id n xs             
...
getPhi :: Block -> (Block, Block) 
getPhi [] 
  = ([], []) 
getPhi (x : xs)
  | s@(Assign _ (Phi _ _)) <- x
    = (s : phis, others)
  | otherwise      
    = (phis, others)
    where 
      (phis, others) = getPhi xs       

unPhi :: Block -> Block
-- Pre: the block is in SSA form 
unPhi (DoWhile (Assign id (Phi e1 e2) : b) e3 : xs)
  = Assign id e1 : b'
  where 
    b' = unPhi (DoWhile (b ++ [Assign id e2]) e3 : xs)   
unPhi (If exp b1 b2 : Assign id (Phi e1 e2) : xs)
  = unPhi (If exp (unPhi b1 ++ [Assign id e1]) (unPhi b2 ++ [Assign id e2]) : xs)
unPhi (If exp b1 b2 : xs) 
  = If exp (unPhi b1) (unPhi b2) : unPhi xs    
unPhi (x : xs)
  = x : unPhi xs  
unPhi [] 
  = []                  
